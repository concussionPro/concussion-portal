...
export interface RenderReportOptions {
  /** Server-stored clinic document profile — renders the letterhead line
   *  (provider block) identically for every seat and device. */
  letterhead?: {
    practitioner?: string
    ahpra?: string
    provider?: string
...
export function renderReportContentToHtml(
  ...
  const lhParts = lh
    ? [
        lh.practitioner,
        lh.ahpra ? `AHPRA ${lh.ahpra}` : '',
        lh.provider ? `Provider no. ${lh.provider}` : '',
        lh.address,
        lh.phone,
      ].filter((x): x is string => Boolean(x && x.trim()))
    : []
  const letterheadLine = lhParts.length
    ? `\n<p class="meta">${lhParts.map(esc).join(' &middot; ')}</p>`
    : ''
