# CEA Partnership & Distribution — lever map + outreach drafts
...
## 1. Affiliate educators — HIGHEST leverage (do first)
Targets: **David Pope (Clinical Edge)**, **Karen Finnin (Online Physio / Off-Script)**, Lachlan Wilmot. They own engaged physio/allied-health audiences. One endorsed send = potentially 50–200 sales. Offer **25–30% rev share**.
> Subject: Concussion CPD for your audience — affiliate?
>
> Hi David,
>
...
## 3. ESSA — exercise physiologists (PITCHED 2026-06-17)
EP-scoped online stream built (8 CPD hours, /demo/essa). Sent to info@essa.org.au / PD team. Awaiting Steve Irwin (PD Coordinator). On the call: confirm CPD-point band for 14h, and — critically — whether they *promote* (Partner PD) vs just *list*. (Full ESSA draft + the EP-stream spec are in this repo's build + memory.)

## 4. Chiropractors — ACA / Chiropractic Australia (PREP)
~6,000 chiros. Near-peer to osteos in scope (MSK, cervical, sports), so the **flagship course fits with minimal adaptation** — no separate stream needed. Pitch ACA endorsement / member access (Chiropractic Board CPD eligible).
**Contact:** ACA national office (info@chiro.org.au) — ask for the Education/CPD or Partnerships lead; or run the Hunter domain-lookup (`/api/admin/hunter-lookup?domain=chiro.org.au`) for a named contact.
> Subject: Concussion CPD for your members
>
...
## 5. Occupational therapists — OT Australia (PREP)
~25,000 OTs. Genuine but **scope-specific** — OTs own the part no one else does: return-to-work and return-to-school, cognitive-fatigue and activity pacing, environmental adaptation. Like the EP play, this needs an **OT-scoped stream** (function & participation, not diagnosis or exercise prescription) — build it *if* they engage. Pitch first.
**Contact:** OT Australia (info@otaus.com.au) — ask for the PD / Professional Practice lead; or Hunter-lookup `otaus.com.au`.
> Subject: Concussion return-to-function CPD — your members are placed for it
>
...
## 6. Guild Insurance — FOLLOW-UP only (waiting to hear back)
You've gone via the OA warm intro (→ Christie Boucher, RiskHQ). Only nudge if quiet ~1 week:
> Subject: Following up — concussion CPD for Guild insureds
>
> Hi Christie, following up on the OA intro. Concussion clearance is now a named responsibility for physios and GPs under the 21-day stand-down — a live medico-legal exposure for clinics that aren't set up. My OA-endorsed program closes that gap. Happy to run a member-discount campaign or contribute concussion risk-reduction content to RiskHQ. Good time this week?
>
> Zac
