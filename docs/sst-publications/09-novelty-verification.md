...
**Verified:** The Rhea concussion rehabilitation app prescribes aerobic exercise at a fixed **60% ± 5% of age-predicted maximal heart rate (220 − age)** and does **not** administer any graded exertion test to measure an individualised threshold; individualisation comes only from in-app "same/better/worse" symptom feedback.
...
**My original assumption ("CCMI doesn't measure HRt") was FALSE.** CCMI's clinic testing software **does** measure HRt via an in-clinic BCTT (their battery: BCTT + SCAT6 + sway + King-Devick + orthostatic vitals). The **Wibbi** module (Dec 2025 partnership) delivers **prescription + real-time symptom/outcome tracking** inside a **closed, certification-gated, proprietary** platform. HRt is a **one-off clinic-administered prescription input**, not a serially re-measured instrument; outcome tracking is symptom/balance scores, not a measured-HRt physiological curve.
...
> "Complete Concussions administers the Buffalo Concussion Treadmill Test to identify a patient's heart-rate threshold in-clinic and, via its December 2025 Wibbi partnership, delivers individualised sub-threshold prescriptions with real-time symptom/outcome tracking inside a certification-gated proprietary platform. SST Trainer differs on three axes: (a) HRt is treated as a *serial, provenance-verified physiological recovery trajectory* rather than a one-off prescription input; (b) the graded test is *clinician-gated but home-capable* rather than clinic-bound; (c) the clinical engine is *purpose-built for the Australian allied-health lane*."
