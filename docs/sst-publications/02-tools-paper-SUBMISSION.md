...
The same separation-of-concerns is applied to output. The clinical core is *payer-neutral*: it produces only measured facts — the serial heart-rate-threshold trajectory, the individualised training band, the verified-session share, and every stop-rule event. A thin **jurisdiction resolver** then maps a locale (AU / NZ / international) to the set of report *skins* a clinician may emit, and each **report skin** is a pure function that renders a structured content model (headings, labelled fields, and narrative) rather than final bytes — so a new report type is a single new function and a new output format (PDF, HTML, or a note written back into a practice-management system) is a single new renderer, neither of which touches the engine. Practice-management-system connectivity sits behind one small **adapter interface** (find patient, read demographics, write note, attach document, and — because no clinical webhooks are assumed — poll appointment attendance), so supporting an additional system is an isolated adapter file registered in one place. Two adapters are implemented against the vendors' documented public APIs: **Cliniko** (Australia), writing the report as a treatment note and filing the rendered PDF through Cliniko's multi-step presigned attachment-upload flow; and **Gensolve** (New Zealand, ACC-integrated), which first resolves the patient's ACC condition and attaches the note to the funded ACC45 claim so the record lands in the payer's own claim context rather than a detached document store. The jurisdiction resolver emits payer-specific skins accordingly — an Australian general-practitioner/return-to-play letter, and, for New Zealand ACC, the content for the **ACC884 Client Summary Report** (the ACC Concussion Service's end-of-service outcome deliverable) plus an explicit *"outcome measures reviewed during delivery"* block that enumerates each graded re-test as a formal review point, so the outcome measures are shown to have been reviewed *across*, not merely at the end of, service delivery. Where a prescribed form governs (the ACC884, or the state workers-compensation forms), the skin supplies the measured content to populate the authoritative form rather than substituting for it; only the general-practitioner letter and the medicolegal record, for which no standard form exists, are rendered as free-form documents. Consistent with the claim discipline of this paper, every skin reports only provenance and signal quality (measured versus manually entered heart rate, verified-session share, and which decisions were clinician-directed); no skin asserts an efficacy or diagnostic claim, and all recovery or clearance language is construct-level and explicitly left to the treating clinician. Verification of this layer is scoped honestly: the write-back request shapes are conformance-tested against mocked endpoints (§7), while validation against a live tenant of each practice-management system is pending and listed among the empirical work in §8.2. The code is closed and no repository link is provided.
...
7. **Report-delivery shape tests** asserting that the practice-management write-back issues the documented request for each adapter (method, endpoint, and body structure) against mocked endpoints, that a failed PDF attachment does not fail the clinical note, and that write-back stays inert unless both an explicit enablement flag and a credential are present. These confirm request-shape conformance only; a live-tenant round-trip against each system is deferred to §8.2.
...
4. **Live-tenant practice-management round-trip.** The Cliniko and Gensolve write-back paths are verified only at the request-shape level (§7); a round-trip against a live tenant of each system — confirming the note and attachment are created, and that the ACC-condition resolution behaves as documented — is required before the integration is enabled in production, and is the reason the write-back ships behind an enablement flag.
