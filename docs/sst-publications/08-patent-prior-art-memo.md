...
| P5 | **CCMI + Wibbi concussion module** — commercial concussion exercise-prescription module inside a home-exercise-program platform with care-pathway builder + outcome tracking; marketed to CCMI clinics incl. Australia | **2025-12-04** | [wibbi.com](https://wibbi.com/resource/complete-concussions-wibbi-partnership/) | Direct commercial prior art / competitor: *digital concussion exercise-prescription delivery with clinician workflow and outcome tracking.* |
...
- **Gated camera-PPG fallback as a safety-bounded source.** A camera-photoplethysmography heart-rate source constrained to **fail closed** — normalised-autocorrelation periodicity detection with a variance gate and a minimum-confidence threshold below which it returns *no value* — used as a fallback within a hardware-agnostic abstraction (BLE / camera-PPG / manual) that drives identical downstream logic. (Note: camera-PPG for HR/HRV is itself widely known, e.g. consumer pacing apps; the potentially-novel angle is the *fail-closed gating tied to a clinical ceiling*, not PPG per se. Counsel should weigh this carefully.)
