...
- Concussion rehab apps already deliver HR-targeted aerobic prescription using **age-predicted maximal HR** — the *Rhea Health* app (a University of Toronto spin-out, founder Michael Hutchison, *not* Buffalo-affiliated) uses a **fixed ~60% ± 5% of 220 − age with no graded test**, individualises only through in-app symptom feedback, and reports no clinician dashboard (Chizuk et al., *JMIR Form Res* 2025); a progressive-%-of-age-predicted-max-HR RCT is also published (*PLOS One*, 2022).
...
Against that, the safest and most honest contribution is **not first-ness but standardisation**: defining a single, repeatable, safety-anchored sequence — and tying it to three specific properties the prior art does not standardise around: a **BCTT-derived (measured) HRt** anchor rather than an age-predicted-max formula; **verification-gated progression** (only live-verified wearable sessions advance the band, which is capped at the measured threshold); and a **fail-closed, no-fabricated-signal** rule (heart rate is shown only when actually measured, else the workflow pauses). The narrow novelty statement for the wider programme — *to our knowledge, no commercially available concussion product delivers a measured (BCTT-derived, not age-predicted-max) heart-rate threshold plus live verification-gated heart-rate-zone training plus a serial measured-HRt clinician trajectory in one tool* — is carried here as a **workflow standard**, which is the lowest-risk form of the claim.
...
3. **Real signal or no signal — and only verified signal advances the dose.** Heart rate is shown only when actually measured (a real BLE packet, or a confidence-gated camera-PPG estimate used as a *resting spot-check only*); the workflow pauses on "no reading" rather than acting on a guess. Progression is stricter still: only a live-verified wearable session may advance the band, which is capped at the measured HRt — manual or unverified sessions still count for safety and regression but never raise the ceiling (see Stage 4).
4. **Hardware-agnostic across platforms.** Any wearable broadcasting the standard Bluetooth Heart Rate profile drives the identical workflow — read via Web Bluetooth on Android/desktop and native CoreBluetooth/Android-BLE in the iOS/Android apps. Camera PPG is a resting spot-check only, and manual entry is always available (including for Apple Watch and Fitbit, which do not broadcast standard BLE).
...
- **Progression decision (clinician-reviewable).** From the recent session history the tool proposes *advance / hold / regress / refer / retest*:
  - **Advance** the ceiling (e.g. +5 bpm) only after a run of clean **live-verified** sessions (no within-session provocation, no next-day flare, ≥80% of prescribed minutes completed). Only sessions whose heart rate was live-verified against a broadcasting wearable count toward the clean run; manual or camera-PPG sessions never advance the band.
  - **Regress** the ceiling on recent repeated provocation (≥2 flares in the recent window), then rebuild. Regression is *never* gated — flare/safety evidence counts from every session, verified or not.
  - **Cap at the measured threshold.** An advance never exceeds the measured HRt; once the band reaches it the tool proposes a **re-test** rather than ratcheting higher — a fresh graded test is the only safe way to raise the band.
  - **Hold** otherwise.
...
| ≥2 flares in recent window (any session) | Repeated provocation — regression never gated | Regress ceiling and rebuild; consider re-test/referral |
| Run of clean **verified** sessions + ≥80% minutes | Tolerance improving (verified sessions only) | Advance ceiling (e.g. +5 bpm), capped at measured HRt |
