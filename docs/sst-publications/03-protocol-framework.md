...
1. **Clinician-owned, not consumer self-managed.** A clinician runs/oversees the threshold test, owns the prescription, and reviews progression and clearance. The tool extends the clinician's reach between visits; it does not replace clinical judgement or enable self-diagnosis.
