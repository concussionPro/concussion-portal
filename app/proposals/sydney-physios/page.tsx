...
const CLINIC = {
  name: 'Sydney Physios and Allied Health Service',
  shortName: 'Sydney Physios',
  city: 'Sydney',
  region: 'Sydney',
  state: 'NSW',
...
export const metadata: Metadata = {
  title: 'Concussion Hub Program — Sydney Physios',
  description: 'Working preview portal for Sydney Physios and Allied Health Service, Sydney NSW.',
  robots: 'noindex, nofollow',
}

export default async function SydneyPhysiosHubPage({
  searchParams,
}: {
    ...
    <div className="min-h-screen dashboard-bg flex items-center justify-center p-6">
      <div className="max-w-md w-full glass-premium rounded-2xl p-8 text-center">
        ...
        <p className="text-sm text-muted-foreground leading-relaxed mb-5">
          Prepared for <strong className="text-foreground">Sydney Physios and Allied Health Service</strong>. Access requires the link from Zac&rsquo;s introductory email.
        </p>
        <a
  ...
  return (
    <div className="hidden md:flex fixed left-0 top-0 h-screen w-64 sidebar-premium p-6 flex-col z-40">
      ...
      <nav className="flex-1 space-y-1">
        ...
        <SidebarItem href={`/proposals/sydney-physios/learning?k=${ACCESS_KEY}`} label="Learning Suite" icon={BookOpen} />
        ...
        <SidebarItem href={`/proposals/sydney-physios/references?k=${ACCESS_KEY}`} label="Reference Library" icon={Library} />
        <SidebarItem href={`/proposals/sydney-physios/toolkit/clinical?k=${ACCESS_KEY}`} label="Clinical Toolkit" icon={FileText} locked />
        <SidebarItem href={`/proposals/sydney-physios/toolkit/outreach?k=${ACCESS_KEY}`} label="Outreach Kit" icon={Stethoscope} locked />
        <SidebarItem href={`/proposals/sydney-physios/toolkit/admin?k=${ACCESS_KEY}`} label="Admin Workflow" icon={BookMarked} locked />
      </nav>
...
function StartHereHero() {
  return (
    <a
      href={`/proposals/sydney-physios/learning?k=${ACCESS_KEY}`}
...
function ProspectBento() {
  return (
    <div className="bento-premium">
      ...
      />
      <Tile
        href={`/proposals/sydney-physios/references?k=${ACCESS_KEY}`}
        icon={Library}
        iconTone="amber"
      ...
      />
      <Tile
        href={`/proposals/sydney-physios/toolkit/clinical?k=${ACCESS_KEY}`}
        icon={FileText}
        iconTone="amber"
      ...
      />
      <Tile
        href={`/proposals/sydney-physios/toolkit/outreach?k=${ACCESS_KEY}`}
        icon={Stethoscope}
        iconTone="amber"
        label="Outreach Kit"
      ...
      />
      <Tile
        href={`/proposals/sydney-physios/toolkit/admin?k=${ACCESS_KEY}`}
        icon={BookMarked}
        iconTone="amber"
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-[10px] uppercase tracking-[0.18em] font-bold text-amber-300">
              The day at your Sydney clinic · highest-value product
            </p>
          </div>
          <h3 className="text-xl sm:text-2xl font-bold mb-1 leading-tight">
...
const COHORT_TIERS = [
  {
    ...
    perClinician: 950,
    total: 9500,
    detail: 'Minimum cohort size. Run the program with your senior physios.',
  },
  {
    name: 'Recommended',
    ...
    perClinician: 900,
    total: 10800,
    detail: `Sweet spot for a physio-led team like ${CLINIC.shortName}.`,
    recommended: true,
  },
  {
    name: 'Full team',
    ...
    perClinician: 870,
    total: 12180,
    detail: 'Whole-team training. Per-clinician rate drops at scale.',
  },
] as const
    ...
    <section id="pricing" className="mt-8 scroll-mt-8">
      <div className="mb-4">
        ...
        <h3 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight">
          On-site training at your Sydney clinic
        </h3>
      </div>
      ...
      <div className="mt-5 glass-premium rounded-xl p-4 flex items-center justify-between gap-4 flex-wrap">
        <p className="text-xs text-muted-foreground leading-relaxed flex-1 min-w-[200px]">
          All prices include GST. Minimum 10; 12–14 is the sweet spot for a team your size. Cohort size confirmed on a 20-min call alongside a date that suits the clinic.
        </p>
        <a
...
const FAQS = [
  ...
  },
  {
    ...
    a: 'For under-10 cohorts, the cleanest path is sending the team to one of our public workshops — same 14 CPD hours, same content, no on-site travel logistics for sub-minimum numbers. Public workshops run regularly in Melbourne, Sydney and Brisbane. We can confirm the next available date on the scoping call.',
  },
  {
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[12px] text-white/85">Grab a slot that works for Sydney Physios.</p>
          </div>
        </div>
