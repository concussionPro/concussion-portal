...
// here. The free module awards a certificate of completion, not CPD hours.
const freeCourseSchema = {
  '@context': 'https://schema.org',
  '@type': 'Course',
  name: 'Concussion Care Has Changed (Free Module)',
  description:
    'Free ~1-hour online awareness module for healthcare professionals on how concussion care has changed — the paradigm shift away from rest toward early sub-symptom-threshold aerobic exercise, recognising concussion and its urgent red flags, what modern management now involves, and where recognition ends and specialist care begins. Certificate on completion. No credit card required.',
  inLanguage: 'en-AU',
  provider: {
    '@type': 'Organization',
  ...
  offers: {
    ...
    url: `${CONFIG.SEO.SITE_URL}/concussion-update`,
  },
  hasCourseInstance: [
    {
      '@type': 'CourseInstance',
  ...
  teaches: [
    'What Changed in Concussion Care',
    'Concussion Recognition and Red Flags',
...
export default function ConcussionUpdatePage() {
  const [name, setName] = useState('')
  const [email, setEmail] = useState('')
  const [error, setError] = useState('')
  // "You already have an account" is a SUCCESS, not a failure — it must not
  // render in the red error box. /scat-mastery and /scat6-download already
  // separate the two; this page is the same signup component forked and was
  // left behind (2026-08-06 route register), so the person most likely to
  // already be a customer saw "Something went wrong" styling on a good outcome.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.requiresEmailLogin) {
        setNotice(
          "You already have an account \u2014 we've emailed a login link to " +
            ...
            ". Open it and you'll be signed straight into the module.",
        )
        return
      }
      ...
      if (data.success) {
        ...
        setSuccessData(data)
        await fireSignupConversionThenNavigate(email.trim().toLowerCase(), '/modules/104')
        return
      } else {
  ...
  const learningPoints = [
    'See exactly what changed in concussion care — the paradigm shift and the evidence behind it',
    'Recognise a suspected concussion and the red flags that need urgent referral',
    'Understand what modern management now involves — and the two clinical roles it splits into',
    'Know where recognition ends and specialist care begins',
    'Finish with a short knowledge check and earn a certificate of completion',
  ]
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-background">
      <BreadcrumbSchema items={[
        { name: 'Home', url: '/' },
        { name: 'Concussion Care Has Changed (Free Module)', url: '/concussion-update' },
      ]} />
      <script
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          <div>
            ...
            <p className="text-lg text-slate-600 mb-4 leading-relaxed">
              In 2022 the international consensus rewrote concussion treatment: rest is out, and early sub-symptom-threshold aerobic exercise is now first-line. Most clinicians practising today were trained on the old rest model. This free module brings you up to date.
            </p>
            <div className="inline-flex items-center gap-2 mb-8 rounded-lg bg-[#5b9aa6]/10 border border-[#5b9aa6]/25 px-4 py-2.5">
              <span className="text-sm font-semibold text-slate-800">
                Recognition, red flags, and the new first-line treatment — with a <span className="text-[#5b9aa6] font-bold">certificate of completion</span>
              </span>
            </div>
        ...
        <div className="mt-20 md:mt-24">
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-[#5b9aa6]/20 p-8 md:p-10 shadow-lg shadow-teal-100/30">
            ...
            <ul className="space-y-3 max-w-md mx-auto mb-8">
              {[
                'Diagnose, phenotype and clinically manage concussion — beyond recognition',
                'SCAT6, SCOAT6, VOMS and BESS assessment with hands-on scoring practice',
