...
//
// Founding-clinic signup for the SST Trainer platform. Replaces the flaky
// mailto CTAs. Persists the lead to Postgres (founding_clinic_interest,
// created lazily on first use — same pattern as lib/early-access.ts) and
// notifies Zac via the existing Resend setup. Best-effort email; the lead is
// committed to the DB before we attempt to notify, so a lead is never lost to
// an email failure.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Founding clinic registered: ${cleanEmail.slice(0, 3)}*** — ${cleanClinic} (${state})`
    )

    return NextResponse.json({
      success: true,
      message: `Thanks ${cleanClinician.split(' ')[0]} — you're on the founding list. Zac will be in touch shortly to set up ${cleanClinic}.`,
    })
  } catch (error) {
...
function buildNotificationEmail(args: {
  ...
  const { clinician, clinic, email, state, volumeLabel, message, isUpdate } = args
  return `
