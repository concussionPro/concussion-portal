 ...
 *
 * EXACTLY the same structure as the CCM trial at /p/[token]/learning/module-1,
 * only the data source differs: the real EP course content (data/ep-modules)
 * instead of data/modules. Same prospect-portal shell, same trial gating (first
 * N sections + quiz checkpoints readable, the rest locked), same nav. No new
 * architecture — a faithful mirror of CCM so both streams behave identically.
 */
...
const PREVIEW_PRINT_CSS = `
  @media print {
    ...
    body::before {
      content: "Module 1 trial preview — full Concussion Rehab Mastery (8 modules) activates with the Hub Program. portal.concussion-education-australia.com";
...
// ─────────────────────────────────────────────────────────────────────────────
// Components — identical to the CCM trial page
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <section className="mt-10 space-y-6">
      <div className="rounded-2xl bg-gradient-to-br from-accent/8 via-accent/5 to-white border border-accent/25 ring-1 ring-accent/15 p-6 sm:p-8">
        ...
        <p className="text-sm text-foreground leading-relaxed mb-5 max-w-xl">
          The next 7 modules cover recognition and scope, the Buffalo treadmill/bike test, sub-symptom-threshold aerobic rehab, phenotype-specific exercise, graded return to sport, persistent symptoms, and documentation. Every exercise physiologist at {clinicShortName} gets full access.
        </p>
