/**
 * CRM (Concussion Rehab Mastery) Module 1 trial preview for prospects.
 *
 * Mirror of the CCM trial at /p/[token]/learning/module-1 but sourced from the
 * EP course (data/ep-modules) instead of the flagship CCM modules, with ESSA
 * framing. Same structure: first N sections + quiz checkpoints are readable
 * (select-none, print-suppressed), the rest are locked. Keeps the two streams
 * identical in shape — Zac 2026-06-26.
 */
...
const PREVIEW_PRINT_CSS = `
  @media print {
    ...
    body::before {
      content: "Module 1 trial preview — full Concussion Rehab Mastery (8 modules, 8 CPD hrs online) activates with the Hub Program. portal.concussion-education-australia.com";
...
export async function generateMetadata({
  ...
  return {
    title: clinic ? `CRM Module 1 trial — ${clinic.shortName}` : 'CRM Module 1 trial',
    description: clinic
      ? `Concussion Rehab Mastery Module 1 trial preview for ${clinic.name}.`
      : 'Concussion Rehab Mastery Module 1 trial preview.',
    robots: 'noindex, nofollow',
  }
}

export default async function ProspectCrmModuleOneTrial({
  params,
  searchParams,
}: {
  params: Promise<{ token: string }>
  ...
  return (
    <div className="flex min-h-screen dashboard-bg">
      ...
      <main className="flex-1 ml-0 md:ml-64">
        <div data-track-section="crm-module-1-trial" className="max-w-3xl mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8">
          <Link
          ...
          <p className="text-[10px] uppercase tracking-[0.18em] font-bold text-accent mb-1">
            CRM · Module 1 of 8 · Trial preview
          </p>
          ...
          <div className="flex items-center gap-3 text-[11px] text-muted-foreground mb-6">
            ...
            <span>·</span>
            <span>ESSA-accredited · pending</span>
          </div>

          {/* Trial content — readable on screen but NOT copyable (select-none)
              and NOT printable (print CSS above). */}
      ...
      <TalkToZacFooter clinicShortName={clinic.shortName} context="crm-module-1-trial" />
    </div>
  )
}
    ...
    <section className="mt-10 space-y-6">
      <div className="rounded-2xl bg-gradient-to-br from-accent/8 via-accent/5 to-white border border-accent/25 ring-1 ring-accent/15 p-6 sm:p-8">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-[10px] uppercase tracking-[0.18em] font-bold text-accent">
            End of CRM Module 1 trial
          </p>
        </div>
        <h2 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight leading-tight mb-3">
          The exercise-rehab arm of concussion care for {clinicRegion}.
        </h2>
        <p className="text-sm text-foreground leading-relaxed mb-5 max-w-xl">
          The next 7 modules cover recognition &amp; scope, the Buffalo treadmill/bike test, sub-symptom-threshold aerobic rehab, phenotype-specific exercise prescription, graded return to sport, persistent symptoms, and EP-scope documentation. Every exercise physiologist at {clinicShortName} gets full access.
        </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-6">
          ...
          <StatPill headline="≥21 days" label="Mandatory stand-down, AIS/SMA community guidelines" />
          <StatPill headline="First-line" label="Active rehab now precedes prolonged rest" />
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
          ...
          <p className="text-[11px] text-muted-foreground">
            CRM goes live for {clinicShortName} once ESSA approval lands · the on-site Hub day is shared with CCM.
          </p>
        </div>
      </div>
      <Link
