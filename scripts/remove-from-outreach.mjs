...
console.log(`\n[1/3] suppression  : ensured (reason=manual-removal-do-not-contact)`)
...
console.log(`[2/3] prospect rows: ${clinicsArchived} archived${clinics.length - clinicsArchived ? ', ' + (clinics.length - clinicsArchived) + ' already terminal' : ''}`)
...
console.log(`[3/3] nurture user : ${usersUnsub ?? 0} updated${users.length && !usersUnsub ? ' (already unsubscribed)' : ''}`)
