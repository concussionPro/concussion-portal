...
interface Usage {
  ...
  /** Course-included platform granted, 3-month clock not yet started. */
...
export function SstClinicCard({
  ...
  // Start the included 3 months (owner 2026-09-11). viewKey-authed server-side;
  // idempotent — an already-running period is never reset or extended.
  const activateIncluded = async () => {
    if (!clinic) return
    setActivating(true)
    setActivateError(null)
    try {
      const res = await fetch('/api/sst/activate-included', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ clinicCode: clinic.code, viewKey: clinic.viewKey }),
      })
      const data = await res.json().catch(() => null)
      if (!res.ok) {
        setActivateError((data && data.error) || 'Activation failed — try again.')
        return
      }
      // Re-read usage so the card flips to the subscribed pill immediately.
      const r2 = await fetch('/api/clinical-testing/clinic', { credentials: 'include' })
      const d2 = await r2.json().catch(() => null)
      if (d2?.usage) setUsage(d2.usage)
    } catch {
    ...
    <div className="glass-premium rounded-2xl p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          {/* PENDING ACTIVATION (owner 2026-09-11): a course purchase grants
              the platform but the included 3 months start when the CLINICIAN
              says so — not at purchase, so the clock never burns while they
              are still doing the course. Until then the trial allowance
              applies, which is enough to try it on a real patient. */}
          {usage?.pendingActivation && (
            <div className="mt-3 rounded-xl border-2 border-teal-300 bg-teal-50 p-4">
              <p className="m-0 text-[13.5px] font-bold text-teal-900">
                Your included 3 months hasn&rsquo;t started yet
              </p>
              <p className="mt-1 mb-0 text-[12px] leading-snug text-teal-800">
                Your enrolment includes 3 months of the full platform. The clock starts when you
                activate — do it when you&rsquo;re ready to run real patients. Until then you can
                trial it on up to {usage.cap ?? 3} patients.
              </p>
              <button
                type="button"
                disabled={activating}
                onClick={activateIncluded}
                className="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-accent px-5 py-2.5 text-[13px] font-bold text-white hover:bg-accent/90 disabled:opacity-60"
              >
                {activating ? 'Activating…' : 'Activate my included 3 months →'}
              </button>
              {activateError && <p className="mt-2 mb-0 text-[12px] font-semibold text-red-700">{activateError}</p>}
            </div>
          )}
          {usage?.plan === 'active' && !usage.pendingActivation && (
            <div className="mt-2 flex flex-wrap items-center gap-2">
